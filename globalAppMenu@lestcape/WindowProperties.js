...
WindowProperties.prototype = {
    ...
    _init: function(window, blacklist, undecorateTile, undecorateAll) {
        this.window = window;
        ...
        this.workspaces = [];
        this.use_set_hide_titlebar = true;
    ...
    onUnmaximise: function(shellwm, actor) {
        ...
        if (!this.use_set_hide_titlebar && global.display.get_grab_op() === Meta.GrabOp.MOVING) {
            ...
            this.grabID = global.display.connect('grab-op-end', function () {
    ...
    onChangeNWorkspaces: function() {
        ...
        while (i--) {
            ...
            //ws._MaximusWindowAddedId = ws.connect('window-added', Lang.bind(this, this.onWindowAdded));
            ws._MaximusWindowAddedId = ws.connect('window-added', function (ws, win) {
                Mainloop.idle_add(Lang.bind(this, function () { 
                    this.onWindowAdded(ws, win);
                    return false; 
                });
            });
        }
    ...
    startUndecorating: function() {
        ...
        if (this.use_set_hide_titlebar && Meta.prefs_get_theme().match(/^(?:Ambiance|Radiance)$/)) {
        ...
        if (!this.use_set_hide_titlebar) {
            if(this.maxID == 0)
                this.maxID = global.window_manager.connect('maximize', Lang.bind(this, this.onMaximise));
            if(this.minID == 0)
                this.minID = global.window_manager.connect('unmaximize', Lang.bind(this, this.onUnmaximise));
            if((this.tileID == 0) && (this.undecorateTile == true))
                this.tileID = global.window_manager.connect('tile', Lang.bind(this, this.onMaximise));
    ...
    stopUndecorating: function() {
        if (this.changeWorkspaceID > 0) global.window_manager.disconnect(this.changeWorkspaceID);
        if (this.maxID > 0) global.window_manager.disconnect(this.maxID);
        if (this.minID > 0) global.window_manager.disconnect(this.minID);
        if (this.tileID > 0) global.window_manager.disconnect(this.tileID);
        if (this.grabID > 0) global.display.disconnect(this.grabID);
        this.changeWorkspaceID = 0;
        this.maxID = 0;
        this.minID = 0;
        this.tileID = 0;
        ...
        while (i--) {
            this.workspaces[i].disconnect(this.workspaces[i]._MaximusWindowAddedId);
            delete this.workspaces[i]._MaximusWindowAddedId;
        }
        this.workspaces = [];
